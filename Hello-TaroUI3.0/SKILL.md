...
7. **按需读取 design token。** 如果组件样式文件只写了 token 名但没有给出值，或实现代码需要精确 CSS 值，只读取 `references/design.md` 中对应章节。
...
- 所有视觉值必须来自组件样式文件或 `design.md` token。不要自行发明近似颜色、圆角、间距或阴影。
...
- `references/design.md`：Hello-CN token 来源。默认只读取需要的 token 章节，不要整篇加载。
