...
- In field-work, outdoor, or high-frequency mobile pages, prefer `large` or `middle` for primary thumb-zone actions.
- In dense management or information-heavy pages, use `small`, `extra small`, `text`, or `outline` only for local row-level actions.
