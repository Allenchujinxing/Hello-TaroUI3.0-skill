...
1. 组件的通用视觉属性优先从本文件 Token 取值。
2. `references/component-styles/` 中记录的 Figma 实测结构值属于组件级例外，优先于本文件通用 token。
3. 页面布局、模块间距和全局校验必须遵循本文件的 12 / 8 / 4 间距规则。
4. 当组件库默认样式与本文件冲突时，以本文件为准；当组件样式文件与本文件通用规则冲突时，以组件样式文件为准。
5. 生成 HTML/CSS 时，用本文件的值定义 CSS 变量（如 `--color-brand: #0076FF`），组件样式中引用变量名。
...
Hello-CN 是哈啰国内业务移动端视觉 token 系统，覆盖 C 端用户产品、B 端运营/作业产品、H5 活动页、Taro 小程序与 App 内 WebView 等移动端场景。

### 设计原则

- **品牌一致：** 以蓝色品牌基调贯穿不同业务线，保证哈啰移动端体验的统一识别。
- **功能优先：** 视觉服务于任务完成，关键内容、状态和操作优先清晰可见。
- **克制清晰：** 控制装饰性表达，使用稳定的留白、层级、状态色和组件结构建立秩序。
- **移动友好：** 兼顾单手操作、户外强光、弱网、定位、支付、作业与高频切换等移动端真实环境。
- **网格统一：** 4px 基础栅格，所有间距尽量使用 4 的倍数。
...
- 标题 600，正文 400
- 10px 主要用于飘带徽标、气泡数字和组件样式文件明确记录的极小文字，不用于正文或普通辅助信息
- 15/13/11 为特殊场景字号，慎用；组件样式文件已记录的实测字号按组件契约执行
...
| shadow-2 | `0 4px 8px 0 rgba(17,17,17,0.08)` | 下拉面板、特殊浮层 / 悬浮卡片、FAB |
...
- 向下：导航栏使用 shadow-1；常规内容卡片默认无投影，仅特殊浮层 / 悬浮卡片可使用 shadow-2
...
`space-card-padding` 仅用于 Card 组件的 body/header 内边距。List、Input、ImageUpload 等组件的容器内边距属于组件 anatomy：上下仍为 16px，左右为 12px，按各组件规范执行。

### 间距序列

2 / 4 / 8 / 12 / 16 / 20 / 24 / 28 / 32 / 36 / 40 / 44 / 48 / 56 / 64 / 80 px

0 / 2px 仅用于线条、对齐、图标或徽标微调例外，不作为页面布局间距。页面 / 模块之间使用 12px，模块内子元素使用 8px，更细的内部紧凑元素使用 4px。
...
| 导航栏总高度 | 88px |
| 底部安全区（iPhone X+） | 34px |
| 底部操作栏 | 内容区 72px + 安全区 34px = 总高 106px |
...
哈啰 TaroUI 3.0 iconfont Symbol 图标库，详见 `references/component-styles/basic/icon.md`。
...
### 投影
- ✅ 常规内容卡片默认无投影；仅特殊浮层 / 悬浮卡片使用 shadow-2；底部栏向上投影
- ❌ 灰底用 shadow-2；投影色用纯黑 rgba(0,0,0,x)

### 布局
- ✅ 页面布局和模块间距遵循 12 / 8 / 4；组件内部实测结构值按组件文件执行；预留安全区
- ❌ 把 0 / 2px 当作布局间距；用页面级间距覆盖组件内部实测 padding / gap；内容被安全区遮挡

### 图标
- ✅ 用哈啰 TaroUI 3.0 iconfont；尺寸与文字搭配；色彩引用 text Token
